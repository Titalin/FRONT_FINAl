...
export default function AdministrarSuscripciones() {
  ...
  useEffect(() => {
    if (!token) { setAlerta('No hay sesión activa.'); return; }
    try {
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} alignItems="center" spacing={2} justifyContent="space-between">
            <Box>
              <Stack direction="row" spacing={1.2} alignItems="center">
                ...
                <Typography variant="h5" fontWeight={800}>Administrar Suscripciones</Typography>
              </Stack>
              <Typography variant="body2" sx={{ opacity: 0.95, mt: 0.5 }}>Activa o gestiona tu plan. Paga de forma segura con PayPal.</Typography>
            </Box>
            <Paper elevation={0} sx={{ p: 2, borderRadius: 2, bgcolor: ui.heroCardBg, border: `1px solid ${ui.borderSoft}`, minWidth: 280, backdropFilter: 'blur(4px)' }}>
              <Stack direction="row" spacing={1.2} alignItems="center">
                <CreditCardIcon fontSize="small" />
                <Typography variant="subtitle2">Estado</Typography>
                <Chip size="small" label={suscripcionActiva ? 'Activa' : 'Inactiva'} color={suscripcionActiva ? 'success' : 'warning'} sx={{ ml: 'auto' }} />
              </Stack>
              {planActual ? (
                <>
                  <Typography variant="body2" sx={{ mt: 1 }}>Plan: <b>{planActual && planActual.nombre}</b></Typography>
                  <Typography variant="body2" sx={{ opacity: 0.85 }}>${toMoney(planActual && planActual.costo)}/mes</Typography>
                </>
              ) : (
                <Typography variant="body2" sx={{ mt: 1, opacity: 0.9 }}>Selecciona un plan para comenzar.</Typography>
              )}
            </Paper>
          </Stack>
        </Paper>
          ...
          {!suscripcionActiva ? (
            <>
              <Stack direction={{ xs: 'column', md: 'row' }} alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
                <Typography variant="h6" fontWeight={700}>Elige un plan para activar tu suscripción</Typography>
                <Button onClick={() => empresaId && cargarDatos(empresaId)} disabled={loading} startIcon={loading ? <CircularProgress size={16} /> : <AutorenewIcon />} variant="outlined" sx={{ color: '#fff', borderColor: ui.borderSoft, '&:hover': { bgcolor: ui.outlineHover, borderColor: '#fff' } }}>
                ...
                {planes.map((plan, idx) => {
                  ...
                  return (
                    <Grid item xs={12} sm={6} md={4} key={String(plan.id)}>
                      <Card elevation={0} sx={{ height: '100%', bgcolor: '#111827', border: `1px solid ${featured ? ui.featuredRing : ui.borderSoft}`, boxShadow: featured ? ui.featuredShadow : 'none', borderRadius: 3 }}>
                        <CardContent>
                          <Stack direction="row" alignItems="center" spacing={1}>
                            <Typography variant="h6" fontWeight={800} sx={{ color: featured ? '#fde68a' : '#93c5fd' }}>{plan.nombre}</Typography>
                            {featured && <Chip label="Recomendado" size="small" sx={{ ml: 'auto', bgcolor: '#f59e0b', color: '#111827', fontWeight: 700 }} />}
                          </Stack>
                          <Typography variant="h3" fontWeight={900} sx={{ color: ui.price, mt: 1, lineHeight: 1 }}>
                            ${toMoney(costo)}
                            <Typography component="span" variant="subtitle2" sx={{ ml: 0.5, opacity: 0.8 }}>/mes</Typography>
                          </Typography>
                          <Typography variant="body2" sx={{ color: ui.mutted, mt: 1 }}>Incluye:</Typography>
                          <List dense sx={{ color: ui.text, pt: 0, pb: 0 }}>
                            <ListItem sx={{ py: 0.5 }}>
                              <ListItemIcon><CheckCircleIcon sx={{ color: '#34d399' }} /></ListItemIcon>
                          ...
                          <Button fullWidth variant="contained" disabled={loading || !HAS_PAYPAL} onClick={() => setPlanSeleccionado(plan)} sx={{ bgcolor: ui.primaryBtn, fontWeight: 800, '&:hover': { bgcolor: ui.primaryBtnHover } }}>
              ...
              {planSeleccionado && paypalOptions && (
                <Box mt={3}>
                      ...
                      <PayPalButtons
                        ...
                        createOrder={async () => {
                          try {
                            ...
                            setAlerta((err && err.response && err.response.data && err.response.data.message) || err.message || 'Error al iniciar el pago.');
                            throw err;
                          }
                        }}
                        onApprove={async (data) => {
                          try {
                            ...
                            setAlerta((e && e.response && e.response.data && e.response.data.message) || e.message || 'Hubo un problema al procesar el pago.');
                          } finally {
                            setLoading(false);
                          }
                        }}
                        onCancel={() => setPlanSeleccionado(null)}
                        onError={() => { setAlerta('Error en el pago con PayPal.'); }}
                      />
            ...
            <>
              <Stack direction={{ xs: 'column', md: 'row' }} alignItems={{ xs: 'flex-start', md: 'center' }} justifyContent="space-between" spacing={1.5} sx={{ mb: 1 }}>
                <Typography variant="h6" fontWeight={800}>Plan actual: {planActual ? planActual.nombre : '-'}</Typography>
                <Button onClick={() => empresaId && cargarDatos(empresaId)} disabled={loading} startIcon={loading ? <CircularProgress size={16} /> : <AutorenewIcon />} variant="outlined" sx={{ color: '#fff', borderColor: ui.borderSoft, '&:hover': { bgcolor: ui.outlineHover, borderColor: '#fff' } }}>
                  {loading ? 'Actualizando…' : 'Refrescar'}
                </Button>
              </Stack>
